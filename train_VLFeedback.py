...
class VLFeedbackDataset(Dataset):
    """
    VLFeedback数据集适配器
    将原始数据集转换为奖励模型训练所需的偏好对格式
    """
    def __init__(self, hf_dataset, processor, split="train", max_samples=None):
        ...
        # 构建偏好对索引
        self.preference_pairs = self._build_preference_pairs()
        
        if max_samples<len(self.preference_pairs):
            self.preference_pairs = self.preference_pairs[:max_samples]

        print(f"构建了 {len(self.preference_pairs)} 个偏好对")
    
    def _compute_weighted_score(self, annotations):
        """
        计算加权综合得分
        annotations: 包含三个维度的评分，每个评分是字符串格式如"5"
        """
        total_score = 0.0
        for aspect, weight in self.aspect_weights.items():
            if aspect in annotations:
                try:
                    ...
                    total_score += weight * 3.0  # 默认中等分数
        return total_score
    
    def _build_preference_pairs(self):
        """
        从数据集中构建偏好对 (chosen, rejected)
        对于每个样本（图像+指令），有4个模型响应和对应的评分
        选择得分最高和最低的响应作为偏好对
        """
        pairs = []
        ...
        for idx, item in enumerate(self.dataset):
            ...
            # 计算每个响应的综合得分
            scored_responses = []
            for i in range(len(completions["annotations"])):
                if(len(completions["response"][i]))<10:
                    continue
                score = self._compute_weighted_score(completions["annotations"][i])
                scored_responses.append({
                    "response": completions["response"][i],
                    "model": completions["model"][i],
                    "score": score
                })
            ...
            # 选择最高分和最低分作为偏好对
            chosen = scored_responses[0]
            rejected = scored_responses[-1]
            
            # 只有当分数有明显差异时才保留
            if chosen["score"] - rejected["score"] > 0.5:
                pairs.append({
...
class RewardTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss = torch.nn.functional.margin_ranking_loss(
            chosen_scores, 
            rejected_scores,
            target=torch.ones_like(chosen_scores),  # 希望chosen > rejected
            margin=margin
        )
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./output/qwen2vl_2b_reward_model",
                        help="Directory to save checkpoints and final model")

    model_name = "Qwen/Qwen2-VL-2B-Instruct"  # 改为2B版本
    output_dir = "./output/qwen2vl_2b_reward_model"
    use_lora = True
    lora_rank = 64
    lora_alpha = 16
    lora_dropout = 0.05
    ...
    max_samples = 30000
    batch_size = 4  # 2B模型可以适当增大batch size（相比7B的2）
    ...
    # 6. 加载VLFeedback数据集
    print("正在加载VLFeedback数据集...")
    hf_dataset = load_dataset("MMInstruction/VLFeedback", split="train")
    print(">>> 数据集加载完成！")
    ...
    print("0")
    # 创建训练数据集
    train_dataset = VLFeedbackDataset(
        hf_dataset, 
        processor, 
        split="train",
    ...
    swanlab_callback = SwanLabCallback(
        project="Qwen2-VL-2B-Reward-1",
        experiment_name="qwen2vl_2b_reward_vlfeedback",
        config={
            "model": model_name,
            "dataset": "VLFeedback",
            "max_samples": max_samples,
    ...
    # 11. 保存模型
    # trainer.save_model()
    # 1. 保存 base_model 或 LoRA adapter
    if use_lora:
        # LoRA 权重已通过 trainer.save_model() 保存？实际上 trainer.save_model() 可能只保存了 base_model，
        # 但 PeftModel 有自己的 save_pretrained 方法，建议手动调用
        reward_model.base_model.save_pretrained(os.path.join(output_dir, "lora_adapter_1"))
    else:
        reward_model.base_model.save_pretrained(os.path.join(output_dir, "base_model"))

    # 2. 始终保存 score_head 权重
    torch.save(reward_model.score_head.state_dict(), os.path.join(output_dir, "score_head_1.pt"))

    tokenizer.save_pretrained(os.path.join(output_dir, "tokenizer_1"))
    processor.save_pretrained(os.path.join(output_dir, "processor_1"))
